...
Sys.timezone() # what timezone I'm in
OlsonNames() # all possible timezones

# force_tz : changes timezone in the datatime
# with_tz : view the datatime in a different timezone
...
##### More on importing and exporting datatimes #####

# parse_date_time() can be slow because it's designed to be forgiving and flexible

# fasttime::fastPOSIXct : faster way to read in ISO 8601 time 
# fast_strptime(x, format = "%Y-%m-%d") - look at base::strptime

# readr::read_csv will read in dates on the UTC time 

# lubridate::stamp will return the string I want it to export the date data 
...
# Parse dates with fast_strptime - used lubridate::stamp to find the format
fast_strptime(dates, 
